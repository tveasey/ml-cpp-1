...
public:
    ...
    void add(core_t::TTime time, double value, double weight, const TPredictor& predictor);
...
private:
    virtual double undonePredict(const TPredictor& predictor, core_t::TTime time) const = 0;
...
public:
    ...
    //! For undo only.
    CLevelShift(core_t::TTime time, double shift, double significantPValue);
...
public:
    CScale(core_t::TTime time,
           ...
           double significantPValue);
    //! For undo only.
    CScale(core_t::TTime time, double scale, double significantPValue);
