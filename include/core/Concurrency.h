...
void setPromise(F& f, P& promise, const std::false_type&) {
    try {
        promise->set_value(f());
    } catch (...) { promise->set_exception(std::current_exception()); }
}
template<typename F, typename P>
void setPromise(F& f, P& promise, const std::true_type&) {
    try {
        f();
        promise->set_value();
    } catch (...) { promise->set_exception(std::current_exception()); }
...
async(CExecutor& executor, FUNCTION&& f, ARGS&&... args) {
    ...
    std::function<void()> task{[g_ = std::move(g), promise_ = std::move(promise)]() mutable {
        concurrency_detail::setPromise(g_, promise_, std::is_same<R, void>{});
}
};
