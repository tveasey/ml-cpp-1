...
CRetrainableModelJsonReader::dataSummarizationFromJsonStream(TIStreamSPtr istream,
    ...
    try {
        return dataSummarizationFromDocumentCompressedImpl(*istream, frame);
    } catch (...) {
...
CRetrainableModelJsonReader::TEncoderUPtrStrSizeUMapPr
CRetrainableModelJsonReader::dataSummarizationFromDocumentCompressedImpl(std::istream& istream,
                                                                         core::CDataFrame& frame) {
    ...
    for (const auto& entry : ifExists(JSON_ENCODING_NAME_INDEX_MAP_TAG, getArray, doc)) {
        encodingIndices.emplace(
            ifExists(JSON_ENCODING_NAME_INDEX_MAP_KEY_TAG, getString, entry),
            ifExists(JSON_ENCODING_NAME_INDEX_MAP_VALUE_TAG, getUint64, entry));
    }
...
CRetrainableModelJsonReader::bestForestFromJsonStream(TIStreamSPtr istream,
    ...
    try {
        return bestForestFromJsonStreamImpl(*istream, encodingIndices);
    } catch (...) {
        // Logging is handled at the point of throw.
    }
    return nullptr;
...
CRetrainableModelJsonReader::TNodeVecVecUPtr
CRetrainableModelJsonReader::bestForestFromJsonStreamImpl(std::istream& istream,
                                                          const TStrSizeUMap& encodingIndices) {
    ...
    for (const auto& trainedModel : trainedModels) {
        ...
        for (const auto& node : treeNodes) {
            ...
            if (node.HasMember(CTree::CTreeNode::JSON_LEAF_VALUE_TAG)) {
                ...
                // Add a split node.
                std::size_t splitFeature{lookup(
                    encodingIndices, featureNames,
                    ifExists(CTree::CTreeNode::JSON_SPLIT_FEATURE_TAG, getUint64, node))};
