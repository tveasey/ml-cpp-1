...
void testChange(const TGeneratorVec& trends,
    ...
    for (std::size_t test = 0; test < 100; ++test) {
        ...
        auto predictor = [&](core_t::TTime time_) {
            return 10.0 * trend(time_);
        };

        maths::CTimeSeriesTestForChange testForChange{
            startTime, startTime, BUCKET_LENGTH, BUCKET_LENGTH, predictor, std::move(values)};
        ...
        if (actualChangeType == expectedChangeType) {
            ...
            meanTimeError(static_cast<double>(std::abs(startTime + 50 * BUCKET_LENGTH - change->time())));
        }
    }
...
BOOST_AUTO_TEST_CASE(testLevelShift) {
    ...
    testChange(
        trends, [](TGenerator trend, core_t::TTime time) { return trend(time) + 0.7; },
...
BOOST_AUTO_TEST_CASE(testScale) {
    ...
    testChange(
        trends, [](TGenerator trend, core_t::TTime time) { return 3.0 * trend(time); },
