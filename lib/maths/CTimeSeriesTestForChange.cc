...
void CChangePoint::add(core_t::TTime time, double value, double weight, const TPredictor& predictor) {
...
CLevelShift::CLevelShift(core_t::TTime time, double shift, double significantPValue)
    : CChangePoint{time, {}, significantPValue}, m_Shift{shift} {
}

CLevelShift::TChangePointUPtr CLevelShift::undoable() const {
    return std::make_unique<CLevelShift>(this->time(), -m_Shift, this->significantPValue());
}
...
double CLevelShift::undonePredict(const TPredictor& predictor, core_t::TTime time) const {
...
CScale::CScale(core_t::TTime time, double scale, double magnitude, TFloatMeanAccumulatorVec residuals, double significantPValue)
    : CChangePoint{time, std::move(residuals), significantPValue},
      m_Scale{std::max(scale, 1e-3)}, m_Magnitude{magnitude} {
}

CScale::CScale(core_t::TTime time, double scale, double significantPValue)
    : CChangePoint{time, {}, significantPValue}, m_Scale{scale} {
}

CScale::TChangePointUPtr CScale::undoable() const {
    return std::make_unique<CScale>(this->time(), 1.0 / m_Scale, this->significantPValue());
}
...
double CScale::undonePredict(const TPredictor& predictor, core_t::TTime time) const {
    return m_Scale * predictor(time);
}
...
operator()(TChangePointUPtr& result, core::CStateRestoreTraverser& traverser) const {
    ...
    if (type == CLevelShift::TYPE) {
        result = std::make_unique<CLevelShift>(time, value, significantPValue);
        return true;
    }
    if (type == CScale::TYPE) {
        result = std::make_unique<CScale>(time, value, significantPValue);
        return true;
    }
