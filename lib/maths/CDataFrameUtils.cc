...
using TFloatUSetVec = std::vector<TFloatUSet>;
using TRowItr = core::CDataFrame::TRowItr;
using TRowSampler = CSampling::CRandomStreamSampler<core::CDataFrame::TRowRef>;
...
CDataFrameUtils::categoryMicWithColumn(std::size_t numberThreads,
    ...
    TFloatUSetVec categories(frame.numberColumns());
    ...
    if (frame.inMainMemory()) {
        ...
        for (auto i : columnMask) {
            ...
            categoryMics.reserve(categories[i].size());
            for (auto category : categories[i]) {
                mic.clear();
                for (const auto& sample : samples) {
                    mic.add(sample[0] != category ? 0.0 : 1.0, sample[1]);
                }
