...
CTimeSeriesSegmentation::constantScalePiecewiseLinearScaledSeasonal(
    ...
    auto computeNoiseStandardDeviation = [&](std::size_t start, std::size_t end) {
        ...
        for (std::size_t i = start; i < end; ++i) {
            if (std::any_of(periods.begin(), periods.end(), [&](const auto& period) {
                ...
                moments.add(constantScale / scale * CBasicStatistics::mean(scaledValues[i]),
                            CBasicStatistics::count(scaledValues[i]));
            }
        }
        return std::sqrt(CBasicStatistics::maximumLikelihoodVariance(moments));
    };
    ...
    for (std::size_t i = 0; i != values.size(); ++i) {
        if (CBasicStatistics::count(values[i]) > 0.0) {
            CBasicStatistics::moment<0>(scaledValues[i]) -=
                scaleAt(i, segmentation, scales) * seasonality(i);
        }
    }

    TDoubleVec noiseStandardDeviations(segmentation.size() - 1);
    for (std::size_t i = 1; i < segmentation.size(); ++i) {
        noiseStandardDeviations[i - 1] =
            computeNoiseStandardDeviation(segmentation[i - 1], segmentation[i]);
    }

    double amplitude{[&] {
        TMinMaxAccumulator minmax;
        for (std::size_t i = 0; i < scaledValues.size(); ++i) {
            minmax.add(constantScale * seasonality(i));
        }
        return minmax.range();
    }()};
    ...
    for (std::size_t i = 0; i < values.size(); ++i) {
        if (std::any_of(periods.begin(), periods.end(),
            ...
            double scale{scaleAt(i, segmentation, scales)};
            double noise{scaleAt(i, segmentation, noiseStandardDeviations)};
            double weight{std::min(amplitude - constantScale / scale * noise, 1.0)};
            if (weight <= 0.0) {
                ...
                CBasicStatistics::moment<0>(scaledValues[i]) *= constantScale / scale;
                CBasicStatistics::moment<0>(scaledValues[i]) += constantScale *
                                                                seasonality(i);
            }
        }
    }
