...
BOOST_FIXTURE_TEST_CASE(testMonitor, CTestFixture) {
...

    {
        ...
        if (sizeof(std::size_t) == 8) {
            // 64-bit platform
            BOOST_REQUIRE_EQUAL(std::size_t(4 * core::constants::BYTES_IN_GB / 2),
                                mon.m_ByteLimitHigh);
        } else {
            // Unexpected platform
    ...
    }
    {
        ...
        if (sizeof(std::size_t) == 8) {
            // 64-bit platform
            BOOST_REQUIRE_EQUAL(std::size_t(4 * core::constants::BYTES_IN_GB),
                                mon.m_ByteLimitHigh);
        } else {
            // Unexpected platform
            BOOST_TEST_REQUIRE(false);
...
BOOST_FIXTURE_TEST_CASE(testExtraMemory, CTestFixture) {
    ...
    monitor.addExtraMemory(core::constants::BYTES_IN_MB);
